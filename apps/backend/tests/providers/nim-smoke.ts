...
export async function run(): Promise<SmokeTestResult> {
  const { NVIDIAProvider } = await import("../../src/services/ai/provider.js");

  const apiKey = process.env.NIM_API_KEY_1;
  if (!apiKey) {
    return { provider: "NVIDIA", status: "skipped", durationMs: 0, returnedKeys: [], error: "NIM_API_KEY_1 not set" };
  }
  ...
  try {
    ...
    return { provider: "NVIDIA", status: "failed", durationMs, returnedKeys: [], error: message };
  }
}
