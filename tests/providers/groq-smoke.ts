...
export async function run(): Promise<SmokeTestResult> {
  const { GroqProvider } = await import("../../src/services/ai/provider.js");

  const apiKey = process.env.GROQ_API_KEY_1 || process.env.GROQ_API_KEY;
  if (!apiKey) {
    return { provider: "Groq", status: "skipped", durationMs: 0, returnedKeys: [], error: "GROQ_API_KEY_1 / GROQ_API_KEY not set" };
  }
  ...
  try {
    ...
    return { provider: "Groq", status: "failed", durationMs, returnedKeys: [], error: message };
  }
}
