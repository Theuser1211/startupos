...
export async function run(): Promise<SmokeTestResult> {
  const { OpenRouterProvider } = await import("../../src/services/ai/provider.js");

  const apiKey = process.env.OPENROUTER_API_KEY;
  if (!apiKey) {
    return { provider: "OpenRouter", status: "skipped", durationMs: 0, returnedKeys: [], error: "OPENROUTER_API_KEY not set" };
  }
  ...
  try {
    ...
    return { provider: "OpenRouter", status: "failed", durationMs, returnedKeys: [], error: message };
  }
}
